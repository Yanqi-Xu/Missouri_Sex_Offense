...
[Missouri State Highway Patrol's website](http://www.mshp.dps.missouri.gov/MSHPWeb/PatrolDivisions/CRID/SOR/SORPage.html) publishes the database along with photos of the offenders to facilitate the public knowledge. Notably, only those who were convicted or offered a plea were registered.

In accordance with Title I of the Adam Walsh Child Protection and Safety Act of 2006, entitled the Sex Offender Registration and Notification Act (SORNA), all 50 states have such a databse to keep track of sex offenders. The [FBI Sex Offender Registry Website](https://www.fbi.gov/scams-and-safety/sex-offender-registry) is a gateway to sex offender registries in all 50 states
...
## Read the data table
The downloadable datasets from the Registry include multiple items. According to the [fact sheet](http://www.mshp.dps.missouri.gov/MSHPWeb/PatrolDivisions/CRID/SOR/factsheet.html),  including name, alias names, date of birth, alias dates of birth, physical description, all addresses, offense, vehicle information, picture of registered sex offenders and offense dates available on the website. We used the two datasets *`msor.xlsx`*, which is a databse of offenders, as well as *`msir_offense`*, which includes more details about individual sex offense cases.
...
```
![](images/slope_graph_02.png)
It shows that offenses against minors constitue about 10% of total offenses, while the offenders' ages vary. 

## Tracking number of sexual offenses
...
Notably, it seems that around 2006-2007, the offenses commited each year started to drop at a remarkable rate. Fore more recent cases, it's understandable that some offenders may have not been convicted, hence the smaller number. Arguably this time period also coincided with the aforementioned SORNA's heightened efforts to expand the registry, "strengthening federal penalties for crimes against children and forms new regional Internet Crimes Against Children Taskforces".

# Mapping the Offenders
Since every offender may be convicted of multiple offenses of the same type on the same day, we may ask where are the offenders per se. I thought it might be interesting looking at te number of offenders versus population, which may shed some light on sex offense crime rate. We can delineate the different steps in it. 

## Prepping the MO_offender data for joining the census data
First off, despite the name, the same offender with the same address could still be listed multiple times. Thus it needs to be grouped by *`Name`* before being grouped by *`County`* to avoid duplicate names. 
```{r}
...
`B0001_001E` is the Census variable name for total estimated population.
```{r key, eval=F}
...
## Calculating percent of Offenders against Population
```{r}
...
```
![](images/MO_map.png)
Voilà! However, there's still blank spaces for some states. For data of this granulality, it seems an interactive map could do much more with the tooltips showing the county names, population and key data. 

This project is part of the mass online class "R for Journalists" taught by the wonderful Washington Post Investigative and Data reporter Andrew Tran.

The idea of slope-chart to showcase correlation is inspired by another MOOC instructor, Alberto Cairo. It does come to me that in one class section, Andrew visualized the age of actress vs. actors using a lollipop chart, which is also very intuitive (it might work better for a small dataset?) 

There's still definitely room for improvement. I am personally very intenrested in facet-mapping the trend overtime per county. I thought of developing a Missouri geofacet() function so that it looks more fun next time I am working on any Missouri map-related Graphic.

Further exploration of this dataset will be updated. 